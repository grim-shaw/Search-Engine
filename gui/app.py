...
def click_search_button(event: Event, result: Text, search_text: Entry, window: Tk) -> None:
    ...
    stemmed_words = [snow_stemmer.stem(word) for word in search_query if word not in stop_words]
    ...
    frame3.pack()
    time_taken_msg = Label(
        frame3,
        text="Search Time (in seconds): ",
        font=("Helvetica", 12, ITALIC),
        background="black",
        foreground="#00FFC0",
    )
    time_taken_msg.pack(side=LEFT)
    time_taken_secs = Label(
        frame3, text=time_taken, font=("Helvetica", 12, ITALIC), foreground="white", background="black"
    )
...
def click_insert_data_button(result: Text) -> None:
    ...
    try:
        ...
        result.delete(0.0, END)
        result.insert(END, "There was an error in generating the forward or inverted index!")
        return

    result.delete(0.0, END)
    if index_info[0]:
        result.insert(END, "Forward and Inverted indices generation successful for json files in " + folder_selected)
        result.insert(END, "\n")
        result.insert(END, "The number of docs scanned were: " + str(index_info[1]))
        result.insert(END, "\n")
        result.insert(END, "Time it took for forward index generation is: " + index_info[2])
        result.insert(END, "\n")
        result.insert(END, "Time it took for inverted index generation is: " + index_info[3])
    else:
        result.insert(
            END, "There were either no json files in the input directory or those json files have already been indexed!"
        )
...
def create_search_window() -> None:
    ...
    Label(window, image=logo, background="black").place(relx=0.5, rely=0.15, anchor=CENTER)

    frame = Frame(window)
    frame.pack()

    search_button = Button(frame, text="Search", font=("Helvetica", 10), width=10, borderwidth=0)
    search_button.pack(side=RIGHT)
    search_button.bind("<Button-1>", lambda event: click_search_button(event, result, search_text, window))
    window.bind("<Return>", lambda event: click_search_button(event, result, search_text, window))
    ...
    add_button = Button(
        ...
        width=11,
        borderwidth=0,
        command=partial(click_insert_data_button, result),
    )
